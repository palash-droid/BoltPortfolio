...
const ResumeModal = ({ isOpen, onClose }: ResumeModalProps) => {
    ...
    const handleOpenInNewTab = (resume: Resume) => {
        ...
        <AnimatePresence>
            {isOpen && (
                ...
                >
                    ...
                    {/* Modal Content */}
                    <motion.div
                        className="relative bg-white dark:bg-gray-900 rounded-lg shadow-2xl max-w-5xl w-full mx-4 max-h-[95vh] overflow-hidden z-[10000]"
                    ...
                    >
                        ...
                        <div className="flex items-center justify-between p-4 sm:p-6 border-b border-gray-200 dark:border-gray-700">
                            ...
                            <div className="flex items-center gap-2 sm:gap-3">
                                <button
                                    ...
                                    disabled={isLoading}
                                    className="flex items-center gap-2 px-3 sm:px-4 py-2 bg-primary-500 text-white rounded-lg hover:bg-primary-600 transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed text-sm"
                                >
                            ...
                            <div className="flex overflow-x-auto scrollbar-hide">
                                {resumes.map((resume) => (
                                    <button
                                        ...
                                        className={`flex-shrink-0 px-4 py-3 text-sm font-medium transition-colors duration-200 whitespace-nowrap ${selectedResume.id === resume.id
                        ...
                        {/* PDF Viewer */}
                        <div className={`p-4 sm:p-6 ${isMobile ? 'h-[65vh]' : 'h-[75vh]'} overflow-hidden`}>
                            {isMobile && showMobileFallback ? (
                                ...
                                <div className="w-full h-full bg-gray-100 dark:bg-gray-800 rounded-lg flex flex-col items-center justify-center p-6">
                                    <div className="text-center space-y-4">
                                        ...
                                        <div className="flex flex-col sm:flex-row gap-3 w-full max-w-sm">
                                            <button
                                                onClick={() => handleOpenInNewTab(selectedResume)}
                                ...
                                <div className="w-full h-full bg-gray-100 dark:bg-gray-800 rounded-lg overflow-hidden">
                                    <iframe
                                        src={`${selectedResume.filePath}#toolbar=0&navpanes=0&scrollbar=0`}
                                        className="w-full h-full border-0"
                                        title={`${selectedResume.title} PDF Viewer`}
                                        onLoad={() => {
                        ...
                        {/* Footer */}
                        <div className="flex items-center justify-between p-4 sm:p-6 border-t border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900">
                            <p className="text-xs sm:text-sm text-gray-600 dark:text-gray-400 flex-1 pr-2">
                                {isMobile
                                    ? "Use the buttons above for best experience."
                                    : `Viewing ${selectedResume.title}. Use the download button to save a copy.`
                                }
                            </p>
                            <div className="flex items-center gap-2 text-xs sm:text-sm text-gray-500 dark:text-gray-400 flex-shrink-0">
